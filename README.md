...
其中服務角色金鑰、Steam Web API 金鑰、同步 token 與 OpenAI 金鑰只能放在 Vercel 的伺服器端環境變數，不可放入 `NEXT_PUBLIC_*`。Supabase Authentication > URL Configuration 已設定正式網站 URL 與 redirect URL。
